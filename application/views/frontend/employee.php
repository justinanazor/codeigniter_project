							...
							<tbody>
								...
								foreach ($employee as $row): ?>
									<tr>
										...
										<td><a href="<?php echo base_url('employees/view/' . $row->id); ?>" class="btn btn-info btn-sm">View</a></td>
										<td><a href="<?php echo base_url('employees/edit/' . $row->id); ?>" class="btn btn-success btn-sm">Edit</a></td>
										<!-- Delete button without Jquery confirmation -->
										<!-- <td><a href="<?php echo base_url('employees/delete/' . $row->id); ?>" class="btn btn-danger btn-sm">Delete</a></td> -->
										<td><button value="<?= $row->id; ?>" data-fullname="<?= $row->full_name ?>" class="btn btn-danger btn-sm confirm-delete">Delete</button></td>
									</tr>
								<?php endforeach; ?>
							</tbody>
						</table>
